...
public class ImageController {
    ...
    public String showImage(@PathVariable("id") String id, Model model, HttpSession session) {
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session) {
        ...
        User user = (User)session.getAttribute("loggeduser"); // Get the loggedin user
        if(user.getId()!=image.getUser().getId()) { // compare loggedin user and image owner
            String error = "Only the owner of the image can edit the image";
            model.addAttribute("editError", error);
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, Model model, HttpSession session) {
        ...
        Image image = imageService.getImage(imageId);
        User user = (User)session.getAttribute("loggeduser");// Get the loogedin user
        if(user.getId()!=image.getUser().getId()) {// compare loggedin user and image owner
            String tags = convertTagsToString(image.getTags());
            model.addAttribute("image", image);
            model.addAttribute("tags", tags);
